...
local Timo = io.open("Timo", 'w')
Timo:write([[
cd $(cd $(dirname $0); pwd)
while(true) do
sudo lua5.3 Timo.lua
done
]])
Timo:close()
...
screen -S ]]..Redis:get(SshId.."Info:Redis:User")..[[ ./Timo
done
]])
Run:close()
...
HELPBEVQ = Redis:sismember(Timo.."HELPBEVQ:Groups",UserId) 
...
return{
BanAll = Redis:sismember(Timo.."BanAll:Groups",UserId) ,
ktmall = Redis:sismember(Timo.."ktmAll:Groups",UserId) ,
BanGroup = Redis:sismember(Timo.."BanGroup:Group"..ChatId,UserId) ,
...
Redis:sadd(Timo.."allM"..msg.chat_id, msg.id)
if Redis:get(Timo.."Status:Del:Media"..msg.chat_id) then    
local Numbardel = Redis:get(Timo.."allM:numdel"..msg.chat_id) or 200
local gmedia = Redis:scard(Timo.."allM"..msg.chat_id)  
if tonumber(gmedia) >= tonumber(Numbardel) then
local liste = Redis:smembers(Timo.."allM"..msg.chat_id)
...
local Num_Msg_Max = Redis:hget(Timo.."Spam:Group:User"..msg_chat_id,"Num:Spam") or 5
...
Num_Msg_Max = Redis:hget(Timo.."Spam:Group:User"..msg_chat_id,"Num:Spam") 
end
end 
...
local N = (Redis:get(Timo.."Name:Bot") or "كلير")
...
if Redis:get(Timo.."Broadcasting:Groups:Pin" .. msg_chat_id .. ":" .. msg.sender.user_id) then 
if text == "الغاء" or text == 'الغاء الامر 𖥔' then   
Redis:del(Timo.."Broadcasting:Groups:Pin" .. msg_chat_id .. ":" .. msg.sender.user_id) 
...
if Redis:get(Timo.."Game:Monotonous"..msg.chat_id) then
if text == Redis:get(Timo.."Game:Monotonous"..msg.chat_id) then
Redis:del(Timo.."Game:Monotonous"..msg.chat_id)
Redis:incrby(Timo.."Num:Add:Games"..msg.chat_id..msg.sender.user_id, 1)  
...
if Redis:get(Timo.."Game:Reflection"..msg.chat_id) then
if text == Redis:get(Timo.."Game:Reflection"..msg.chat_id) then
Redis:incrby(Timo.."Num:Add:Games"..msg.chat_id..msg.sender.user_id, 1)  
Redis:del(Timo.."Game:Reflection"..msg.chat_id)
...
Redis:del(Timo.."SADD:NUM"..msg.chat_id..msg.sender.user_id)
...
Redis:incrby(Timo.."SADD:NUM"..msg.chat_id..msg.sender.user_id,1)
if tonumber(Redis:get(Timo.."SADD:NUM"..msg.chat_id..msg.sender.user_id)) >= 3 then
Redis:del(Timo.."SADD:NUM"..msg.chat_id..msg.sender.user_id)
Redis:del(Timo.."Game:Estimate"..msg.chat_id..msg.sender.user_id)
...
os.execute('wget https://adfrv.zzz.com.ua/beb/Timo.lua')
...
local StatusMember = LuaTele.getChatMember(bana.id,Timo).status.luatele
...
LuaTele.sendText(msg_chat_id,msg_id,'*𖥔 عدد احصائيات البوت الكامله \n⩹━━━━𖥕Timo𖥕━━━━⩺\n𖥔 عدد المجموعات : '..(Redis:scard(Timo..'ChekBotAdd') or 0)..'\n𖥔 عدد المشتركين : '..(Redis:scard(Timo..'Num:User:Pv') or 0)..'*',"md",true)  
end
...
return LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد الاعضاء قليل لا يمكن تفعيل المجموعه  يجب ان يكوم اكثر من :'..Redis:get(Timo..'Num:Add:Bot'),"md",true)  
end
...
if not Redis:sismember(Timo.."DevelopersQ:Groups",bana.id) then
...
if not Redis:sismember(Timo.."Developers:Groups",bana.id) then
...
if not Redis:sismember(Timo.."TheBasicsQ:Group"..msg_chat_id,bana.id) then
...
if not Redis:sismember(Timo.."TheBasics:Group"..msg_chat_id,bana.id) then
...
if not Redis:sismember(Timo.."Originators:Group"..msg_chat_id,bana.id) then
...
if not Redis:sismember(Timo.."Managers:Group"..msg_chat_id,bana.id) then
...
if not Redis:sismember(Timo.."Addictive:Group"..msg_chat_id,bana.id) then
...
if not Redis:sismember(Timo.."Distinguished:Group"..msg_chat_id,bana.id) then
...
Redis:sadd(Timo.."DevelopersQ:Groups",bana.id) 
...
Redis:sadd(Timo.."Developers:Groups",bana.id) 
...
Redis:sadd(Timo.."TheBasicsQ:Group"..msg_chat_id,bana.id) 
...
Redis:sadd(Timo.."TheBasics:Group"..msg_chat_id,bana.id) 
...
Redis:sadd(Timo.."Originators:Group"..msg_chat_id,bana.id) 
...
Redis:sadd(Timo.."Managers:Group"..msg_chat_id,bana.id) 
...
Redis:sadd(Timo.."Addictive:Group"..msg_chat_id,bana.id) 
...
Redis:sadd(Timo.."Distinguished:Group"..msg_chat_id,bana.id) 
...
local Info_Members = Redis:smembers(Timo.."BanGroup:Group"..msg_chat_id) 
if #Info_Members == 0 then
...
keyboardd.inline_keyboard = {
{
{text = '< Developers Timo  >', url = "https://t.me/Q_o_ll"}
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
local Added_Me = Redis:get(Timo.."Who:Added:Me"..msg.sender.user_id..':'..msg.sender.user_id)
if Added_Me then 
...
if not msg.ControllerBot and not Redis:set(Timo.."LeftBot") then
...
Redis:set(Timo..''..lock..msg_chat_id,"del")    
end
...
list ={"Lock:Bot:kick","Lock:User:Name","Lock:hashtak","Lock:Cmd","Lock:Link","Lock:forward","Lock:Keyboard","Lock:geam","Lock:Photo","Lock:Animation","Lock:Video","Lock:Audio","Lock:vico","Lock:Sticker","Lock:Document","Lock:Unsupported","Lock:Markdaun","Lock:Contact","Lock:Spam"}
for i,lock in pairs(list) do 
Redis:del(Timo..''..lock..msg_chat_id)    
end
...
{
{text = 'اتعطيل ردود السورس', data = msg.sender.user_id..'/'.. 'rb:bna_ban'},{text = 'اتفعيل ردود السورس', data = msg.sender.user_id..'/'.. 'rb:bna_Timo'},
},
{
...
if Redis:get(Timo.."Status:BanId"..msg.chat_id)  then
...
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد رسائلك هنا *~ '..(Redis:get(Timo..'Num:Message:User'..msg.chat_id..':'..msg.sender.user_id) or 1)..'*',"md",true)  
elseif text == 'سحكاتي' or text == 'تعديلاتي' then
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد التعديلات هنا *~ '..(Redis:get(Timo..'Num:Message:Edit'..msg.chat_id..msg.sender.user_id) or 0)..'*',"md",true)  
elseif text == 'جهاتي' then
LuaTele.sendText(msg_chat_id,msg_id,'𖥔 عدد جهاتك المضافه هنا *~ '..(Redis:get(Timo.."Num:Add:Memp"..msg.chat_id..":"..msg.sender.user_id) or 0)..'*',"md",true)  
...
Redis:del(Timo.."BanGroup:Group"..msg_chat_id) 
...
if Statusrestricted(msg_chat_id,Message_Reply.sender.user_id).BanGroup == true then
BanGroup = 'محظور ,'
Redis:srem(Timo.."BanGroup:Group"..msg_chat_id,Message_Reply.sender.user_id) 
else
BanGroup = ''
end
if Statusrestricted(msg_chat_id,Message_Reply.sender.user_id).SilentGroup == true then
...
BanAll = 'محظور عام ,'
Redis:srem(Timo.."BanAll:Groups",bana.id) 
else
BanAll = ''
end
if Statusrestricted(msg_chat_id,bana.id).BanGroup == true then
BanGroup = 'محظور ,'
Redis:srem(Timo.."BanGroup:Group"..msg_chat_id,bana.id) 
else
BanGroup = ''
end
if Statusrestricted(msg_chat_id,bana.id).SilentGroup == true then
SilentGroup = 'مكتوم ,'
Redis:srem(Timo.."SilentGroup:Group"..msg_chat_id,bana.id) 
else
SilentGroup = ''
end
LuaTele.sendText(msg_chat_id,msg_id,"\n*𖥔 تم رفع القيود عنه : {"..BanAll..BanGroup..SilentGroup..Restricted..'}*',"md",true)  
...
local Namebot = (Redis:get(Timo.."Name:Bot") or "كلير")
local BotName = {
...
if text == (Redis:get(Timo.."Name:Bot") or "كلير") then
local photo = LuaTele.getUserProfilePhotos(Timo)
local ban = LuaTele.getUser(Timo)
local Namebot = (Redis:get(Timo.."Name:Bot") or "كلير")
local BotName = {
...
Redis:set(Timo.."Game:Monotonous"..msg.chat_id,name)
...
if text == "العكس" then
if Redis:get(Timo.."Status:Games"..msg.chat_id) then
Redis:del(Timo.."Set:Aks"..msg.chat_id)
katu = {"باي","فهمت","موزين","اسمعك","احبك","موحلو","نضيف","حاره","ناصي","جوه","سريع","ونسه","طويل","سمين","ضعيف","شريف","شجاع","رحت","عدل","نشيط","شبعان","موعطشان","خوش ولد","اني","هادئ"}
name = katu[math.random(#katu)]
Redis:set(Timo.."Game:Reflection"..msg.chat_id,name)
...
local CmdStart = '*\n𖥔 أهلآ بك في بوت '..(Redis:get(Timo.."Name:Bot") or "كلير")..
...
keyboardd.inline_keyboard = {
{
{text = '< Developers Timo  >', url = "https://t.me/M_P_L_6"}
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
keyboardd.inline_keyboard = {
{
{text = '< Developers Timo  >', url = "https://t.me/BoOdY_BaSha"}
},
}
local msg_id = msg.id/2097152/0.5 
 https.request("https://api.telegram.org/bot"..Token..'/sendPhoto?chat_id='..msg.chat_id..'&caption='..URL.escape(TestText)..'&photo='..photo.photos[1].sizes[#photo.photos[1].sizes].photo.remote.id..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboardd))
...
if not msg.ControllerBot then
if Redis:get(Timo.."TwaslBot") and not Redis:sismember(Timo.."BaN:In:Tuasl",msg.sender.user_id) then
...
Redis:set(Timo.."rank_tf"..IdUser,"rank_tk") 
end
end 
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:del(Timo.."Status:BanId"..ChatId) 
local reply_markup = LuaTele.replyMarkup{type = 'inline',data = {{{text = ': رجوع ➡️ .', data =UserId..'/'.. 'listallAddorrem'},},}}
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Timo.."Status:BanId"..ChatId,true) 
local reply_markup = LuaTele.replyMarkup{type = 'inline',data = {{{text = ': رجوع ➡️ .', data =UserId..'/'.. 'listallAddorrem'},},}}
...
elseif Text and Text:match('(%d+)/rb:bna_Timo') then
local UserId = Text:match('(%d+)/rb:bna_Timo')
if tonumber(IdUser) == tonumber(UserId) then
Redis:set(Timo.."rb:bna"..ChatId,true) 
local reply_markup = LuaTele.replyMarkup{type = 'inline',data = {{{text = ': رجوع ➡️ .', data =UserId..'/'.. 'listallAddorrem'},},}}
...
if tonumber(IdUser) == tonumber(UserId) then
Redis:del(Timo.."BanGroup:Group"..ChatId) 
